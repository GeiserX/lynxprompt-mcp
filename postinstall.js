...
const REPO = "GeiserX/lynxprompt-mcp";
const BIN_NAME = process.platform === "win32" ? "lynxprompt-mcp.exe" : "lynxprompt-mcp";
...
function getAssetName() {
  ...
  return `lynxprompt-mcp_${VERSION}_${platform}_${arch}.${ext}`;
}
...
async function main() {
  if (fs.existsSync(BIN_PATH)) {
    console.log("lynxprompt-mcp binary already exists, skipping download.");
    return;
  }
  ...
  console.log(`Downloading lynxprompt-mcp v${VERSION} for ${process.platform}-${process.arch}...`);
  ...
  console.log(`Installed lynxprompt-mcp to ${BIN_PATH}`);
}

main().catch((err) => {
  console.error(`Failed to install lynxprompt-mcp: ${err.message}`);
  process.exit(1);
});
